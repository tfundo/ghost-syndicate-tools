/* ===================================================
   ...
   Usa Auth global · Verifica membresía servidor Ghost Syndicate
   =================================================== */
'use strict';
...
window.Market = (function () {
  ...
  function mount(elId) {
    ...
    if (!_initialized) {
      _initialized = true;
      window.Auth?.onUserChange(async (user) => {
        if (user) {
          _inGuild = null;
          render();
          await checkGuild();
    ...
    if (_inGuild !== null) { render(); return; }
    render();
    checkGuild().then(async () => { if (_inGuild) await loadListings(); render(); });
  }

  // ── Auth (delega a Auth global) ───────────────────────────
  function login()  { window.Auth?.login(); }
  function logout() { window.Auth?.logout(); }

  // ── Guild check ───────────────────────────────────────────
  async function checkGuild() {
    const session = window.Auth?.getSession();
    if (!session?.provider_token) { _inGuild = false; return; }
    try {
  ...
  async function deleteListing(id) {
    ...
    await sb.from('listings').update({ active: false }).eq('id', id).eq('user_id', user.id);
  ...
  function renderLogin() {
    return `<div class="mkt-gate">
      <div class="mkt-gate-icon">🏪</div>
      <h2 class="mkt-gate-title">${tr('Mercado de Jugadores','Player Market')}</h2>
      <p class="mkt-gate-desc">${tr('Compra, vende e intercambia con la comunidad Ghost Syndicate.<br>Exclusivo para miembros del servidor.','Buy, sell and trade with the Ghost Syndicate community.<br>Server members only.')}</p>
      <button class="mkt-discord-btn" onclick="Market.login()">${DC_SVG} ${tr('Entrar con Discord','Sign in with Discord')}</button>
    </div>`;
  }

  function renderChecking() {
    return `<div class="loading-state"><div class="loading-spinner"></div>
      <p>${tr('Verificando membresía del servidor...','Checking server membership...')}</p></div>`;
  }

  function renderNoGuild() {
    const user = window.Auth?.getUser();
    const name = esc(user?.user_metadata?.full_name || user?.user_metadata?.user_name || '');
    return `<div class="mkt-gate">
      <div class="mkt-gate-icon">🔒</div>
      <h2 class="mkt-gate-title">${tr('Acceso Restringido','Access Restricted')}</h2>
      <p class="mkt-gate-desc">${tr(`Hola <strong>${name}</strong>, este mercado es exclusivo para miembros de <strong>Ghost Syndicate Gaming</strong>. Únete al servidor para acceder.`,`Hello <strong>${name}</strong>, this market is for <strong>Ghost Syndicate Gaming</strong> members only.`)}</p>
      <a class="mkt-discord-btn" href="https://discord.com/invite/Ktfnbmj5s7" target="_blank" rel="noopener">${DC_SVG} ${tr('Unirse a Ghost Syndicate Gaming','Join Ghost Syndicate Gaming')}</a>
  ...
  function renderMarket() {
    ...
    if (_myOnly) shown = shown.filter(l => l.user_id === user?.id);

    const gridHtml = _loadingData
      ? `<div class="loading-state"><div class="loading-spinner"></div></div>`
      : !shown.length
      ? `<div class="empty-state"><div class="empty-icon">📭</div><p>${tr('No hay anuncios en esta categoría','No listings in this category')}</p></div>`
      : `<div class="mkt-grid">${shown.map(l => `
          <div class="mkt-card">
            <div class="mkt-card-thumb">
              ${l.image_url ? `<img src="${esc(l.image_url)}" alt="${esc(l.title)}" loading="lazy">` : `<div class="mkt-no-img">◈</div>`}
            ...
            <div class="mkt-card-body">
              ...
              <div class="mkt-card-foot">
                ${l.discord_avatar ? `<img class="mkt-avatar" src="${esc(l.discord_avatar)}" alt="">` : `<div class="mkt-avatar mkt-avatar-ph">◈</div>`}
                <span class="mkt-card-user">${esc(l.discord_username)}</span>
                <span class="mkt-card-time">${timeAgo(l.created_at)}</span>
                ${l.user_id === user?.id ? `<button class="mkt-del-btn" onclick="Market.deleteListing('${l.id}')" title="${tr('Eliminar','Delete')}">✕</button>` : ''}
              </div>
            </div>
          </div>`).join('')}
        </div>`;
    ...
    return `
      <div class="mkt-topbar">
        ...
        <div class="mkt-top-actions">
          <button class="mkt-mine-btn${_myOnly?' active':''}" onclick="Market.toggleMine()">${tr('Mis anuncios','My listings')}</button>
          <button class="btn-primary mkt-post-btn" onclick="Market.openForm()">+ ${tr('Publicar','Post')}</button>
        </div>
      </div>
      <div class="mkt-filters">
        ${['all','venta','compra','cambio'].map(f => `
          <button class="mkt-filter-btn${_filter===f?' active':''}" onclick="Market.setFilter('${f}')">${fltIcons[f]} ${fltLabels[f]}</button>`).join('')}
        <span class="mkt-count">${shown.length} ${tr('anuncios','listings')}</span>
      </div>
      ${gridHtml}
      ${_showForm ? renderForm() : ''}`;
  }

  function renderForm() {
    return `<div class="mkt-overlay" onclick="if(event.target===this)Market.closeForm()">
      <div class="mkt-modal">
        ...
        <form class="mkt-form" onsubmit="Market.submitForm(event)">
          ...
          <div class="mkt-field">
            ...
            <input id="mktTitle" class="mkt-input" type="text" name="title" placeholder="${tr('Ej: Aurora MR, 100 Widowmaker...','E.g.: Aurora MR, 100 Widowmaker...')}" maxlength="100" required autocomplete="off">
          </div>
          <div class="mkt-field">
            <label for="mktDesc">${tr('Descripción','Description')}</label>
            <textarea id="mktDesc" class="mkt-input mkt-textarea" name="description" placeholder="${tr('Condiciones, estado, cómo contactar...','Condition, details, contact...')}" maxlength="500"></textarea>
          </div>
          ${_formType !== 'cambio' ? `
          <div class="mkt-field">
            <label for="mktPrice">${tr('Precio (aUEC)','Price (aUEC)')}</label>
            <input id="mktPrice" class="mkt-input" type="text" name="price" placeholder="${tr('Ej: 150000 o negociable','E.g.: 150000 or negotiable')}" maxlength="50">
          </div>` : ''}
          <div class="mkt-field">
            <label for="mktImg">${tr('Imagen (máx 2MB)','Image (max 2MB)')}</label>
            <input id="mktImg" class="mkt-file-input" type="file" name="image" accept="image/jpeg,image/png,image/webp,image/gif">
          </div>
