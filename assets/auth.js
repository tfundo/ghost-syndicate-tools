...
window.Auth = (function () {
  ...
  const LS_KEY = 'gst_res';   // localStorage key for offline resources

  let sb         = null;
  let _user      = null;
  let _session   = null;
  let _resources = new Map();   // "category:key" → quantity (always in sync with LS when offline)
  let _listeners = [];
  let _saveTimers = {};
  ...
  function _lsLoad() {
    try {
      ...
      Object.entries(obj).forEach(([k, v]) => _resources.set(k, +v));
    } catch(e) {}
  }
  ...
  function _lsSave() {
    try {
      const obj = {};
      _resources.forEach((v, k) => { if (v > 0) obj[k] = v; });
  ...
  // ── Init ─────────────────────────────────────────────────
  function init() {
    _lsLoad();   // always load local data first so Wikelo works without login
  ...
  async function _loadResources() {
    ...
    if (data && data.length > 0) {
      // Supabase has data → use it as truth, overwrite local
      _resources.clear();
      data.forEach(r => _resources.set(`${r.category}:${r.resource_key}`, +r.quantity));
      _lsSave();
    } else {
      // Supabase empty but we have local data → upload it silently
      const localEntries = [..._resources.entries()].filter(([,v]) => v > 0);
      if (localEntries.length > 0) {
        const rows = localEntries.map(([k, qty]) => {
          const [category, ...rest] = k.split(':');
          return { user_id: _user.id, category, resource_key: rest.join(':'), quantity: qty, updated_at: new Date().toISOString() };
        });
        sb.from('user_resources').upsert(rows, { onConflict: 'user_id,category,resource_key' }).then();
      }
    }
  ...
  function setResource(category, key, qty) {
    ...
    // Always persist locally — works without login
    _lsSave();
    _showSaveStatus('ok');
    ...
    _saveTimers[mapKey] = setTimeout(async () => {
      ...
      if (error) {
        ...
        _showSaveStatus('error', 'Error al guardar en servidor: ' + error.message);
      }
    }, 700);
  }
